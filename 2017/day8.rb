...
instructions.each do |i|
  ...
  regCheck = reg[vars[4]]
  checkAgainst = vars[6].to_i

  runIns = false
  case vars[5]
    when '<'
      runIns = regCheck < checkAgainst
    when '>'
      runIns = regCheck > checkAgainst
    when '>='
      runIns = regCheck >= checkAgainst
  ...
  if runIns
    ...
    reg[vars[0]] += addVal
    if reg[vars[0]] > overallMax
      overallMax = reg[vars[0]]
    end
  end
end
...
puts "highest final value is #{max}"
puts "highest overall value is #{overallMax}"
