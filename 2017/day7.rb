...
rows.each do |r|
  parse = r.split("\s")
  @children[parse[0]] = []
  @nodes[parse[0]] = parse[1].scan(/\d/).join.to_i

  next if parse.length == 2 
  
  childs = parse[3,1337].map{|x| x.scan(/[a-zA-Z]/).join}
  childs.each do |c|
    @children[parse[0]] << c
  end
end
...
parent = testnodes.keys[0]
puts "Parent node is called #{parent}"
...
@children.each do |k, v|
  ...
  v.each do |c|
    ...
    if shouldBe == -1 
      ...
      answer = @nodes[c] + (shouldBe - w)
    end
  end
end

puts "error in #{error} - Should be #{answer}"
